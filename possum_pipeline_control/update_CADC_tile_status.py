...
def update_one_band(band_number, CADC_session, ps):
    ...
    tile_numbers = np.unique(
        [x.split("_")[-2] for x in result["observationID"] if x.startswith(freq)]
    )
    ...
    for tile in tile_numbers:
        ...
        if not full:
            ...
            if w.size != 1:
                raise Exception(
                    f"Could not uniquely find tile row for {tile}. Either missing or duplicated?"
                )
            dt = [
                datetime.datetime.fromisoformat(x)
                for x in tile_products["lastModified"]
            ]
            last_modified = max(dt)

            # as of >v6.0.0 .update requires a list of lists
            tile_sheet.update(
                range_name=f"{col_letter}{int(w[0]) + 2}",
                values=[[last_modified.date().isoformat()]],
            )
            sleep(1)
